 ...
 *
 */
public class Optimator2Graph extends TreeMap<Integer, Optimator2SubGraph> {
	 ...
	 *         Fehler beim Import
	 */
	public static Optimator2Graph importiereGraph(File directory) throws ImportException {
		Optimator2Graph optimatorGraph = new Optimator2Graph();
		...
		try (Scanner scanner = new Scanner(new File(directory, "Graph_Gruppen.txt"))) {
			...
			for (int index : indizes) {
				optimatorGraph.put(index,
						Optimator2SubGraph.importiereGraph(new File(directory, "Graph_" + index + ".xml")));
			}
